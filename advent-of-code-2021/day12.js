...
the caves visited by that path are listed in the order they are visited and separated by commas.)

Note that in this cave system, cave d is never visited by any path:
to do so, cave b would need to be visited twice (once on the way to cave d
  and a second time when returning from cave d), and since cave b is small,
  this is not allowed.
...
a single small cave can be visited at most twice, and the remaining small
caves can be visited at most once. H
owever, the caves named start and end can only be visited exactly once each:
...
Expected output: 116692
*/

//Result based on: https://github.com/constb/aoc2021/tree/master/12

let data = require("fs").readFileSync("day12-input.txt", { encoding: "utf-8" }).trim();
...
function path2Find(pos, current) {
  if (pos === "start" && current.length > 0) return;
  if (pos === "end") {
    found2.push([...current, pos]);
    return;
  }
  const next = [...current, pos];
  ...
  for (const dir of paths[pos])
    if (!hasLowDup || dir.toLowerCase() !== dir || current.indexOf(dir) === -1) path2Find(dir, next);
}

path2Find("start", []);

console.log(`Part 1: ${found.length}`);
console.log(`Part 2: ${found2.length}`);
