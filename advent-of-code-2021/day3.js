...
/* Resolution based on: https://github.com/TheAngularGuy/aoc2021/blob/master/day3/part1.js */

/* tranforms txt file into usable data */
const fs = require('fs');
const input = fs.readFileSync('day3-input.txt').toString().split('\n');

const mostCommonBits = input[0].split('').map(() => ({one: 0, zero: 0}));

/* Sums each each 1 or 0 values */
input.forEach(num => num.split('').forEach(
  (bit, index) => ((bit === '0') ? mostCommonBits[index].zero += 1 : mostCommonBits[index].one += 1)
));
//console.log("most common bits " +  mostCommonBits[0].one + " " + mostCommonBits[0].zero);

/* parseInt(value, base) transforms binary in decimal */
...
/*
Expected Output:
Gamma Rate - Binary: 100111000110, Decimal: 2502
Epsilon Rate 011000111001, Decimal: 1593
Final Answer (Decimal): 3985686
*/
